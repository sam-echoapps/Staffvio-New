...
function (oj,ko,$, app, ojconverterutils_i18n_1, ArrayDataProvider,  ojknockout_keyset_1,ResponsiveUtils, ResponsiveKnockoutUtils, AsyncRegExpValidator) {
    ...
    class StaffTimesheetClientListViewModel {
        constructor(args) {
            var self = this;
            ...
            self.shiftListTimesheet = function (event,data) {
                var clickedRowId = data.data.id
                console.log(clickedRowId)
                sessionStorage.setItem("clientId", clickedRowId);
                self.router.go({path:'timeSheet'})
            }
        }
    }
    return  StaffTimesheetClientListViewModel;
});