define(['ojs/ojcore', 'knockout', 'jquery', 'appController',
    'ojs/ojtranslation', 'ojs/ojconverter-number', 'ojs/ojconverterutils-i18n', 'ojs/ojconverter-datetime', 'ojs/ojarraydataprovider',"ojs/ojpagingdataproviderview",
    ...
    'ojs/ojformlayout', 'ojs/ojradioset','ojs/ojvalidationgroup'],
    function (oj, ko, $, app, Translations, NumberConverter, ConverterUtilsI18n, DateTimeConverter, ArrayDataProvider,PagingDataProviderView) {
        ...
        class ManageViewModel {
            constructor(args) {
                ...
                self.summaryData1 = ko.observableArray([]);
                self.summaryTable = ko.observable();
            ...
            self.stopAllProc = function(data,event){
                    ...
                    if (self.selectedMenuItem() == 'extrpt' || self.selectedMenuItem() == 'extchk' || self.selectedMenuItem() == 'extstats' || self.selectedMenuItem() == 'extstartdef' || self.selectedMenuItem() == 'extstop' || self.selectedMenuItem() == 'extforcestop' || self.selectedMenuItem() == 'extkill' || self.selectedMenuItem() == 'cachemgr' || self.selectedMenuItem() == 'extstatus') {
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                ...
                                document.querySelector('#ViewExtractRptDialog').open();
                                self.ExtRpt(data[0]);
                                var TblArr = Object.keys(data[2]);
                                var dataArr = Object.keys(data[2][TblArr[0]]);
                                self.summaryTable(TblArr[0]);
                               
                                console.log(TblArr);
                                //console.log(dataArr);
                                //console.log(data[2][TblArr[0]][dataArr[1]]['TotalDel']);

                                self.summaryData([]);
                                self.summaryData1([]);

                                // for(var j=0;j<2;j++){
                                //     self.summaryDataDP = ko.observable(new ArrayDataProvider(self.summaryData, {idAttribute: "TabName"}));
                                // }
                                    for(var i=0;i<4;i++){
                                        self.summaryData.push({'Name' : dataArr[i] , 'TotalDel' : data[2][TblArr[0]][dataArr[i]]['TotalDel'] , 'TotalDsc' : data[2][TblArr[0]][dataArr[i]]['TotalDsc'] , 'TotalIns' : data[2][TblArr[0]][dataArr[i]]['TotalIns'] , 'TotalOp' : data[2][TblArr[0]][dataArr[i]]['TotalOp'] ,'TotalUpd' : data[2][TblArr[0]][dataArr[i]]['TotalUpd'] , 'TotalUps' : data[2][TblArr[0]][dataArr[i]]['TotalUps'] }) 
                                       //self.summaryData.push({'TabName' : "test" , 'expRows' : "test" , 'impRows' : "test"}) 
                                    }
                                    
                                    for(var i=0;i<4;i++){
                                        self.summaryData1.push({'Name' : dataArr[i] , 'TotalDel' : data[2][TblArr[1]][dataArr[i]]['TotalDel'] , 'TotalDsc' : data[2][TblArr[1]][dataArr[i]]['TotalDsc'] , 'TotalIns' : data[2][TblArr[1]][dataArr[i]]['TotalIns'] , 'TotalOp' : data[2][TblArr[1]][dataArr[i]]['TotalOp'] ,'TotalUpd' : data[2][TblArr[1]][dataArr[i]]['TotalUpd'] , 'TotalUps' : data[2][TblArr[1]][dataArr[i]]['TotalUps'] }) 
                                       //self.summaryData.push({'TabName' : "test" , 'expRows' : "test" , 'impRows' : "test"}) 
                                    }
                ...
                self.summaryDataDP = ko.observable(new ArrayDataProvider(self.summaryData, {idAttribute: "TabName"}));
                self.summaryDataDP1 = ko.observable(new ArrayDataProvider(self.summaryData1, {idAttribute: "TabName"}));
                
                self.summaryDatacolumnArray = [{headerText: 'Name',
                field: 'Name'},
                {headerText: 'TotalDel',
                field: 'TotalDel'},
                {headerText: 'TotalDsc',
