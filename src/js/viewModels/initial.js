        ...
        function (ko, $,app,AsyncRegExpValidator, ConverterUtilsI18n, DateTimeConverter, NumberConverter, PagingDataProviderView,ArrayDataProvider,ListDataProviderView,ojkeyset_1) {
            ...
            class InitialLoadViewModel {
                ...
                self.SelectSRCDeployment = (event,data) =>{
                 ...
                 //Redshift
                 self.aws_bucket_name = ko.observable('');
                 self.redshiftAlias = ko.observable('');

                 self.openAWSKenisis = function(){
                     if (self.currentBDA() == 'Amazon Kinesis'){
                        self.dialogTitle('Amazon Kinesis Streams Handler Parameters');
                     }
                     else if (self.currentBDA() == 'Amazon Redshift'){
                         getDB();
                        self.dialogTitle('Amazon Redshift Event Handler Parameters');
                     }
...
//Get the Credentail Alias Domains
                ...
                var parallelism = ko.observableArray([
                    {label: '1', value: '1'},
                    {label: '2', value: '2'},
                    {label: '3', value: '3'},
                    {label: '4', value: '4'},
                ...
                self.parallelismDP = new ArrayDataProvider(parallelism, {keyAttributes: 'value'});

                self.srcExpParallel = ko.observable();
...
self.addCSVILProc = function (data, event) {
    self.AddExtractMsg([]);
    self.dialogTitle('Preparing Processes for InitialLoad');
    console.log(self.currentAWSRegion())
    ...
    $.ajax({
        ...
        data: JSON.stringify({
            ...
            parallel : self.srcExpParallel(),
            currentAWSRegion : self.currentAWSRegion(),
            aws_access_key_id : self.aws_access_key_id(),
            aws_secret_access_key : self.aws_secret_access_key(),
            currentAWSBucket : self.aws_bucket_name(),
            currentBDA : self.currentBDA()
        }),
        dataType: 'json',
        timeout: sessionStorage.getItem("timeInetrval"),
        context: self,
...
self.addAutoILProc = function (data, event) {
    ...
    if(self.TgtonepDepType() == 'bda'){
        if (self.currentBDA() == 'Amazon Kinesis'){
            ...
            currentBDA : self.currentBDA(),
            currentAWSRegion : self.currentAWSRegion(),
            kenisis_stream_name : self.kenisis_stream_name(),
            aws_access_key_id : self.aws_access_key_id(),
            aws_secret_access_key : self.aws_secret_access_key(),
            kinesisProxyServer : self.kinesisProxyServer(),
            kinesisProxyServerPort : self.kinesisProxyServerPort() })
        }
    }
    else if(self.TgtonepDepType() == 'oracle'){
        var restData = JSON.stringify({
            srcdep : self.SrcOnePDepName(),
            srcdomain: self.selectedSRCDomCategory(),
            srcalias: self.SRCcurrentPDB(),
    ...
    self.currentRepParam = ko.computed( { 
      ...
      self.currentRepParamList = self.currentRepParam;
                ...
                self.addInitialLoadRep = function (data, event) {
                        ...
                        self.rsAlias = ko.observable('');
                        self.DBDet=ko.observableArray([]);
                        function getDB() {
                              $.ajax({
                                url: self.TGTonepDepUrl() + "/dbdet",
                                type: 'GET',
                                dataType: 'json',
                                context: self,
                                error: function (e) {
                        ...
                        self.rsAliasDP = new ArrayDataProvider(self.DBDet, {keyAttributes: 'value'});
