    ...
    function (oj, ko, $,app, ArrayDataProvider, keySet, AsyncRegExpValidator) {
        class AddDeployViewModel {
            constructor(context) {
                ...
                self.ggDepDBVer = ko.observable();
                self.ggDepGGHome = ko.observable();
                self.ggDepDBHome = ko.observable();
                self.uploadState = ko.observable(true);
                self.groupValid = ko.observable();
                self.CancelBehaviorOpt = ko.observable('icon');
                self.softwareVer = ko.observable();
                ...
                self.softVerDisable = ko.observable(false);
                self.softwareArray = ko.observableArray([]);
            ...
            if (self.onepDepUrl().length > 0) {
             ...
             $.ajax({
               ...
               success: function (data) {
                 ...
                 temp = temp.replace(/,/g, ".");
                 self.ggDepDBClientVer(temp);
                 self.ggDepGGVer(data[4]);
                 self.ggDepDBVer(data[5]);
                 self.ggDepGGHome(data[6]);
                 self.ggDepDBHome(data[7]);
                 if((self.ggDepGGVer() != "Not Installed") || (self.ggDepDBVer() != "Not Installed") || (self.ggDephName() == "")){
                    self.uploadState(true);
                    self.softVerDisable(true);
                  }
                  else{
                    self.softVerDisable(false);
                    self.softwareArray(data[8]);
                  }
                   return self;
               }
        ...
        self.softwareListDP = new ArrayDataProvider(self.softwareArray, { keyAttributes: 'value' });
                
                  
        self.downloadSoftMsg = ko.observableArray([]);

        self.downloadSoftware  = function (data,event){
            if (self.softwareVer() ) {
            self.downloadSoftMsg([]);
            document.querySelector('#downloadSoftwaredialog').open();
                    $.ajax({
                        url: self.onepDepUrl() + "/downloadsoft",
                        type: 'POST',
                        dataType: 'json',
                        data: JSON.stringify({
                            softVer : self.softwareVer(),
                            dbVer: self.ggDepDBClientVer()
                        }),
                         error: function (xhr, textStatus, errorThrown) {
                                if(textStatus == 'timeout' || textStatus == 'error'){
                                    document.querySelector('#downloadSoftwaredialog').close();
                                    document.querySelector('#Timeout').open();
                                    }
                            },
                        success: function (data) {
                            document.querySelector('#downloadSoftwaredialog').close();
                            document.querySelector('#ggSetup').open();
                            for (var i = 0; i < data[0].length; i++) {
                                self.downloadSoftMsg.push(data[0][i]);
                            }
                            self.softVerDisable(true);
                           return self;
                        }
...
self.ggSetupMsgOKClose = function () {

document.querySelector('#ggSetup').close();
getDepDet();
