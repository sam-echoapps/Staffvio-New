    ...
    class RequestedShiftsModel {
        constructor(args) {
            ...
            function getShiftsInfo(){
                self.ShiftPostDet([]);
                $.ajax({
                    url: BaseURL + "/jpGetRequestedShifts",
                    type: 'POST',
                    data: JSON.stringify({
                    ...
                    success: function (result) {
                        ...
                        if(data.length !=0){ 
                            for (var i = 0; i < data.length; i++) {
                                ...
                                self.ShiftPostDet.push({'id': data[i][0],'client_name': data[i][3],'shift_name': data[i][4], 'shift_date' : data[i][5], 'start_time': formattedStartTime, 'end_time' : formattedEndTime });
                            }
                        }
                }
                }) 
            ...
            self.rejectAllocation = function (event,data) {
                var clickedRowId = data.data.id
                    document.querySelector('#openAllocationProgress').open();
                    $.ajax({
                        url: BaseURL + "/jpStaffRejectAllocation",
                        type: 'POST',
                        data: JSON.stringify({
                            rowId : clickedRowId,
                            staffId : sessionStorage.getItem("staffId")
