        ...
        constructor(context) {
            ...
            self.staffJobRoleFilter = function (event,data) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        var photo;
                         for (var i = 0; i < data[0].length; i++) {
                            ...
                            /* if(data[0][i][0] == data[0][i][19]){
                                if(data[0][i][18] == self.client_name()){
                                self.StaffDet.push({'id': data[0][i][0], 'title' : data[0][i][1], 'firstName' : data[0][i][2] , 'lastName' : data[0][i][3] , 'name' : data[0][i][2] + " " +data[0][i][3] ,  'mainPostion' :data[0][i][4] , 'subPostion' : data[0][i][5] , 
                    ...
                    console.log(myArray);
                    let duplicates = findDuplicateStaffIDs(myArray);
                    console.log(duplicates);
                    console.log(duplicates[0].client_id);
                    console.log(duplicates[0].staff_id);
                    let filteredData = removeObjects(myArray, duplicates[0].staff_id, duplicates[0].client_id);
                    console.log(filteredData);
                    let obj = Object.fromEntries(filteredData);
            ...
            function findDuplicateStaffIDs(data) {
                let staffIDs = new Set();
                console.log(staffIDs)
                let duplicateRows = [];
              
                data.forEach(([index, item]) => {
                  if (item.staff_id && staffIDs.has(item.staff_id)) {
                    duplicateRows.push(item);
                  } else {
                    staffIDs.add(item.staff_id);
                  }
                });
              ...
              function removeObjects(data, staffID, clientID) {
                return data.filter(([index, item]) => item.staff_id !== staffID || item.client_id === clientID);
            }
